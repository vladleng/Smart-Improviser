...
Статус: **ACCEPTED** — Влад подтвердил живой тест 2026-09-24. Windows Build #231 — success; PR #23 слит в main. Последняя принятая stable — 0.3.
...
- [x] C major: Dm7 → G7 → Cmaj7. На G7: `dominant to major`, материал G B D F, target Cmaj7.
- [x] В том же C major заменить цель на Cm: `dominant to minor`, target Cm. Глобальная тональность остаётся C major.
- [x] A7 → Dm7: цель minor, secondary относительно глобального C major; локальная трактовка Stage 2 сохраняется.
- [x] Db7 → Cmaj7: SubV, major target; материал содержит реальные pitch classes Db F Ab B (B здесь enharmonic Cb).
- [x] G7 → C7: dominant chain, не major tonic.
- [x] G7 → Abmaj7 или G7 без следующего аккорда: resolution unconfirmed; ложной Cmaj7 цели нет.
- [x] G7sus4: нет добавленной B; C triad: нет добавленной B/b7; G7b9: Ab не исчезает.
- [x] PLAY / STOP / seek и редактирование Chord Track обновляют карточку вместе с Stage 2.
- [x] При отсутствии валидного key/chord/position карточка показывает ожидание контекста; старые рекомендации не остаются.
- [x] Reopen проекта восстанавливает анализ; поля Stage 1–2 работают как прежде.
- [x] Правая карточка читаема и не перекрывает диагностику.
